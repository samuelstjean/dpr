...
function draw_fancy_graph(pval, coords1, coords2, truncated_coords1, truncated_coords2, average1, average2, params)
    ...
    for idx = 1:size(coords1, 1)
        ...
        x = x(isfinite(x));
        z = z(isfinite(z));
        p1 = plot(x, z, 'color', shadow_cmap, 'LineWidth', 5);
    ...
    for idx = 1:size(truncated_coords1, 1)
        ...
        x = x(isfinite(x));
        z = z(isfinite(z));
        p1 = plot(x, z, 'color', bundle_cmap, 'LineWidth', 5);
        p1.Color(4) = 0.3; % transparency
    end

    % Draw the mean coord
    plot(average1, average2, 'color', mean_fiber_cmap, 'LineWidth', 5) %, 'zorder', 5)

    % Again, we only interpolate on non nans, the padding is magically kept, don't ask why
    valid = find(~isnan(pval));
    new_coord = 1:length(x);
    old_coord = 1:length(pval);
